 ...
 * Date : 210829
 */
public class BJ17142_Laboratory3 {
	...
	static int bfs(int[][] simulMap) {					// 너비 우선 탐색 - virusNum만큼의 바이러스들이 동시에 퍼지므로 FIFO 구조가 필요
		...
		for(int i = 0 ; i < virus.size() ; i++) {		// 선택된 활성 바이러스들을 큐에 추가
			if(selected[i]) {
				...
				queue.add(new Point(virus.get(i).i, virus.get(i).j, 0));
			}
		}
		while(!queue.isEmpty()) {						// 큐가 빌 때까지 반복
			...
			for(int d = 0 ; d < 4 ; d++) {				// 위아래왼오 에 위치한 칸에 대해 검사
				...
				if(ni >= 0 && ni < N && nj >= 0 && nj < N && (simulMap[ni][nj] == 0 || simulMap[ni][nj] == -2)) {	// 맵을 벗어나지 않고, 해당 칸이 0이거나 비활성 바이러스라면
					simulMap[ni][nj] = now.time + 1;	// 시간을 저장
					simulTime = Integer.max(simulTime, now.time + 1);		// simulTime을 최댓값으로 갱신
					queue.add(new Point(ni, nj, now.time + 1));				// 큐에 시간을 1 더해 저장
				}
			}
		}
