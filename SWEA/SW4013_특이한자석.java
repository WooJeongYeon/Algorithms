 ...
 * Level : SWEA
 * Thinking)
 * 	배열은 돌리기 귀찮을거같아서 ArrayList로 앞에 뒤에 뺐다꼈다 함
 * 	회전수만큼 계속 돌림(시뮬레이션)
 * Method : 매 회전마다) 회전방향 구함 - 회전
 * 			결과 계산
 * Result : 논리 써놓고서 했더니 바로 통과! 넘좋당
 */
public class SW4013_특이한자석 {
	static int TC, K, ans;
	static ArrayList<Integer>[] magnetics;				// 자석들의 자성정보
	static int[] dirInfo;								// 방향 정보
	static final int MAGNUM = 4;						// 자석의 개수
	...
	public static void main(String[] args) throws NumberFormatException, IOException {
		...
		for(int tc = 1 ; tc <= TC ; tc++) {				// TC만큼 반복
			...
			magnetics = new ArrayList[MAGNUM];
			ans = 0;
			for(int i = 0 ; i < MAGNUM ; i++) {								// 자석들의 자성정보 입력받음
			...
			for(int i = 0 ; i < K ; i++) {									// K번만큼 회전
				st = new StringTokenizer(br.readLine(), " ");
				dirInfo = new int[MAGNUM];									// 회전 후의 방향을 저장할 배열
	...
	static void getDir(int start) {							// 회전했을 때의 각 자석들의 회전할 방향을 저장
		for(int i = start - 1 ; i >= 0 ; i--) {				// 시작점을 기준으로 왼쪽
			...
			else break;										// 같은 극이면 더이상 진행할 필요X
		}
		for(int i = start + 1 ; i < MAGNUM ; i++) {			// 시작점을 기준으로 오른쪽
	...
	static void rotation() {								// 해당 방향으로 회전
		for(int i = 0 ; i < MAGNUM ; i++) {					// 모든 자석에 대해
			int tmp;
	...
	static void calc() {									// 결과를 계산
		for(int i = 0 ; i < MAGNUM ; i++) {
			ans += ((magnetics[i].get(0) & 1) << i);		// 0번째 날의 각 자석의 점수 - 1, 2, 4, 8(2의 배수)
		}
	}
}
